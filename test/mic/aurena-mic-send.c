...
/*
 * A simple RTP server 
 *  sends the output of alsasrc as opus encoded RTP on port 5000,
 *  RTCP is sent on port 5001. The destination is 127.0.0.1.
 *  the receiver RTCP reports are received on port 5005
 *
 */
...
static GstElement *
request_aux_sender (GstElement * rtpbin G_GNUC_UNUSED,
    guint sessid, gpointer userdata G_GNUC_UNUSED)
{
  GstElement *rtx, *bin;
  GstPad *pad;
  gchar *name;
  ...
  g_print ("creating AUX sender for session %u\n", sessid);
...
*/
  g_signal_connect (rtpbin, "request-aux-sender",
      (GCallback) request_aux_sender, NULL);
  ...
  g_assert (rtcpsink);
  g_object_set (rtcpsink, "port", 5001, "host", DEST_HOST, NULL);
